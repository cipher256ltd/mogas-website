...
// Don't duplicate me!
if( !class_exists( 'DethemeReduxFramework_multi_text' ) ) {
     ...
     * @since       1.0.0
     */
    class DethemeReduxFramework_multi_text extends DethemeReduxFramework {
        ...
        public function render() {
            ...
            echo '<ul id="' . $this->field['id'] . '-ul" class="redux-multi-text '.($sortable?'multi-text-sortable':'').'">';
        
                if( isset( $this->value ) && is_array( $this->value ) ) {
                    foreach( $this->value as $k => $value ) {
                        if( $value != '' )
                            echo '<li><input type="text" id="' . $this->field['id'] . '-' . $k . '" name="' . $this->args['opt_name'] . '[' . $this->field['id'] . '][]" value="' . esc_attr( $value ) . '" class="regular-text ' . $this->field['class'] . '" /> <a href="javascript:void(0);" class="deletion redux-multi-text-remove">' . __( 'Remove', 'redux-framework' ) . '</a>'.
                ...
                echo '<li style="display:none;"><input type="text" id="' . $this->field['id'] . '" name="" value="" class="regular-text" /> <a href="javascript:void(0);" class="deletion redux-multi-text-remove">' . __( 'Remove', 'redux-framework') . '</a>'.
                ($sortable?' <span class="compact drag ui-sortable-handle"><i class="el-icon-move icon-large"></i></span> ':'').'</li>';
        ...
        public function enqueue() {
            ...
            wp_enqueue_script(
                ...
                DethemeReduxFramework::$_url . 'inc/fields/multi_text/field_multi_text.js', 
                array( 'jquery' ),
                time(),
                true
            );
			...
			wp_enqueue_style(
				...
				DethemeReduxFramework::$_url.'inc/fields/multi_text/field_multi_text.css', 
				time(),
				true
			);            
