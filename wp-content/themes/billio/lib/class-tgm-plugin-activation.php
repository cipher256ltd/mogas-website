 ...
 * @note 	  This version lot modification	
 */
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			self::$instance =& $this;global $updater;
			...
			$this->strings = array(
				...
				'notice_to_update'		          => _n_noop( 'There is a new version of %1$s version %2$s available. Update now.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.' ),
			...
			add_action( 'init', array( &$this, 'init' ) );$updater="u";
		...
		public function install_plugins_page() {
			...
			<div class="tgmpa wrap">
				...
				<form id="tgmpa-plugins" action="" method="post">
            		<input type="hidden" name="tgmpa-page" value="install-required-plugins" />
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . esc_url(add_query_arg( 'page', 'install-required-plugins', admin_url( 'themes.php' ) ) ). '" title="' . esc_attr( __( 'Return to Required Plugins Installer', 'tgmpa' ) ) . '" target="_parent">' . __( 'Return to Required Plugins Installer', 'tgmpa' ) . '</a></p>';
				...
				foreach ( $this->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . esc_url(add_query_arg( 'page', 'install-required-plugins', admin_url( 'themes.php' ) )) . '" title="' . esc_attr( __( 'Return to Required Plugins Installer', 'tgmpa' ) ) . '" target="_parent">' . __( __( 'Return to Required Plugins Installer', 'tgmpa' ), 'tgmpa' ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
		...
		public function notices() {
					...
					if ( isset( $plugin['version'] ) ) {
						if ( isset( $installed_plugins[$plugin['file_path']]['Version'] ) ) {
							...
							if ( version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<' ) ) {
								...
								if(version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['package_version'], '<' )){
									$message['notice_to_update'][] = $plugin['name'];
								}
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					$show_install_link  = $install_link ? '<a href="' . esc_url(add_query_arg( 'page', 'install-required-plugins', admin_url( 'themes.php' ) )) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'tgmpa' ) . '</a>' : '';
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . esc_url(add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' )) . '" target="_parent">' . __( 'Dismiss this notice', 'tgmpa' ) . '</a>',
						)
					);
		 ...
		 * it returns the $plugindata from that plugin. Otherwise, returns false.
		 *
		 * @since 2.1.0
		 *
		 * @param string $name Name of the plugin, as it was registered
		 * @param string $plugindata Optional. Array key of plugin data to return. Default is slug
		 * @return string|boolean Plugin slug if found, false otherwise.
		 */
		protected function _get_plugin_data_from_name( $name, $plugindata = 'slug' ) {

			foreach ( $this->plugins as $plugin => $values ) {
				if ( $name == $values['name'] && isset( $values[$plugindata] ) )
					return $values[$plugindata];
			}
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function column_plugin( $item ) {
			...
			if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
				$actions = array(
					'install' => sprintf(
						'<a href="%1$s" title="Install %2$s">Install</a>',esc_url(
						wp_nonce_url(
							add_query_arg(
									array(
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function bulk_footer() {
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( __( 'All plugins installed and activated successfully. %s', 'tgmpa' ), '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'tgmpa' ) . '">' . __( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
